...
## 2026-07-15 — Fix the two migrations that never ran on a fresh database, in place
**Chosen:** Edit the two existing migration files rather than add corrective ones: `20260602_102342.ts` now opens with `CREATE SCHEMA IF NOT EXISTS "payload"`, and `20260629_152734_scroll_animation_presets.ts` converts each `scroll_animation` column to `text` (dropping its default first) before remapping `reveal-up`/`reveal-right`, letting the existing per-table type swap restore the enum and default.
**Alternatives:** Add a new migration that creates the schema (cannot work — it would still sort after the init migration that needs it); create the schema from `docker/entrypoint.sh` before `payload migrate`; regenerate the whole migration chain from the current schema; guard the remap with a check that skips it when the target enum value is absent.
**Why:** Both bugs share one cause: every migration was generated against a dev database that a schema push had already shaped, so the files encode assumptions that only hold there. The init migration creates every object in the `payload` schema but nothing creates the schema — Payload only does that during a push — so a never-pushed database fails on statement one with 42P06. The scroll-animation remap sets `'fade-up'` while the column is still the OLD enum, which has no such value; Postgres resolves an enum literal at PLAN time, so it raises 22P02 even with zero matching rows. It therefore failed on exactly the databases it was written for (old enum) and passed only where it was pointless (already-pushed columns). Editing in place is safe because both files are already recorded as applied on every existing database, so they will never re-run there; a fresh database gets the corrected version. Creating the schema from the entrypoint was rejected because it puts a schema-shaped decision outside the migration chain, where `payload migrate` run by hand or by a non-Docker deploy would not get it. Verified by running the built image against a pristine Postgres: 111 tables, all 37 `scroll_animation` columns back on their own enum type with defaults intact, none left as `text`, and a second boot no-ops and serves.
**Trade-offs:** Editing applied migrations means any database that already ran them keeps whatever state it has — the files no longer describe what those databases actually did. Anyone holding a checksum/hash of the old files sees drift. The `DROP DEFAULT` + `SET DATA TYPE text` loop touches every table with a `scroll_animation` column, so a table carrying that column but absent from the type-swap list below it would be left as `text`; none exist at this point in the chain, and the verification query above asserts it.
**Revisit if:** The migration chain is ever regenerated from scratch, or a `scroll_animation` column is added to a table that the swap list does not cover — re-run the drift query against a fresh database.
## 2026-07-17 — Admin Google-provider gate temporarily bypassable via flag
**Chosen:** User decision: gate `getAdminUser`'s Google-provider requirement behind `ALLOW_CREDENTIALS_ADMIN` (`lib/feature-flags.ts`, default `false`), and set it to `true` only in the VPS Portainer stack env for admin testing.
**Alternatives:** SSH tunnel to the VPS so Google OAuth runs on `localhost` (allowed by Google); set up domain + TLS first and keep the gate intact; remove the provider check outright.
**Why:** The production deploy runs on a raw IP (`http://116.118.6.30:3000`) and Google's OAuth console refuses raw public IPs as redirect URIs, so no session can ever satisfy the Google-provider requirement there — the admin panel is unreachable and `/login?callbackUrl=/admin` loops. The user needs admin access now for testing; a default-false flag keeps the secure behavior everywhere the variable is not explicitly set, and reverting is deleting one env line rather than a code change.
**Trade-offs:** While the flag is set, admin access is protected only by the password of the `ADMIN_EMAILS` account over unencrypted HTTP — no Google 2FA in front. The register-then-race escalation the provider check closes is dormant only because the admin email's credentials account already exists.
...
## 2026-07-29 — Spotlight auto mode sources every on-sale product, not just the job's picks
**Chosen:** User decision: a `source` radio on the Spotlight block (`manual` | `auto`). In `auto`, slides come from `getPayloadOnSaleProducts` — every product with `onSale = true AND salePercent > 0` — ordered deepest discount first, capped at 6, with all slide text derived from the product and no countdown.
**Alternatives:** Sourcing only `autoSaleManaged` products (the nightly job's five); a checkbox on the existing deals array rather than a mode selector; ordering by most-recently-discounted or by unique viewers; a shared block-level heading/CTA/countdown applied across auto slides.
**Why:** "On sale" is the shopper-visible fact — a carousel that showed only the job's picks would silently hide a hand-set seasonal discount, which is the opposite of what a deal spotlight is for, so the block deliberately does not read `autoSaleManaged`. A mode selector beats a checkbox because the two modes have genuinely disjoint inputs: in auto there is nothing to type into the per-deal rows. Deepest-discount-first was chosen over most-recently-discounted because there is no sale-start timestamp and `updatedAt` is bumped by any edit, so fixing a typo would push a stale sale to the first slide — the one slide most shoppers actually see. Ordering by views was rejected as it costs a Prisma query on a render path and makes the order drift with traffic. Deriving every field from the product (rather than adding shared overrides) keeps one renderer for both modes, so they cannot drift apart visually; a single shared countdown was rejected because it would imply every sale ends at the same moment, which is false.
**Trade-offs:** Auto mode gives up all editorial voice — no custom headline, no urgency timer, and a carousel that reads as a product list rather than a curated deal. The 6-slide cap silently drops deeper inventory during a large seasonal sale. Because the block hides itself when nothing is discounted, a page laid out around it changes shape without warning. Sale changes surface within the 60s `CATALOG_REVALIDATE` TTL rather than instantly, since the nightly job's writes do not tag-revalidate (see the 2026-07-29 correction entry above).
